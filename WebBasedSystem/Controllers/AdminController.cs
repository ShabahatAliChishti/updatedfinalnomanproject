...
namespace WebBasedSystem.Controllers
{
    public class AdminController : Controller
    {
        ...
        {
            List<NotesModel> ObjListwaitingtime = new List<NotesModel>()
            {
            ...
            ViewBag.WaitingTime = new SelectList(ObjListwaitingtime, "Waiting_Time_Id", "Waiting_Time");
        ...
        [HttpPost]
        public ActionResult Specialist(CommonNotes_SpecialistModel model)
        {
            try
            {
                List<NotesModel> ObjListwaitingtime = new List<NotesModel>()
                ...
                List<SpecialistModel> ObjList = new List<SpecialistModel>()
            {
                ...
                var findtitle = ObjList.Where(x => x.TitleId == model.specialistmodel.TitleId).FirstOrDefault();

                var getwaitingtime = ObjListwaitingtime.Where(x => x.Waiting_Time_Id == model.notesmodel.Waiting_Time_Id).FirstOrDefault();

                var submenu = db.SpecialistTypeSubMenus.FirstOrDefault(x=>x.SubMenuId==model.specialistmodel.SubMenuId);

                Specialist spec = new Specialist();

                spec.Title = findtitle.Title;
                spec.SpecialistName = model.specialistmodel.SpecialistName;
                spec.MobileNo = model.specialistmodel.MobileNo;
                spec.PhoneNo = model.specialistmodel.PhoneNo;
                spec.Address1 = model.specialistmodel.Address1;
                spec.Age = model.specialistmodel.Age;
                spec.Suburbs = model.specialistmodel.Suburbs;
                spec.State = model.specialistmodel.State;
                spec.PostCode = model.specialistmodel.PostCode;
                spec.SpecialistTypeId = model.specialistmodel.SpecialistTypeId;
                spec.SubMenu = submenu.SubMenuName;
                spec.Address2 = model.specialistmodel.Address2;
                spec.EmailAddress = model.specialistmodel.EmailAddress;
                spec.BusinessName = model.specialistmodel.BusinessName;
                spec.Surname = model.specialistmodel.Surname;
                spec.OfficeNo = model.specialistmodel.OfficeNo;
                spec.IsDeleted = false;
                db.Specialists.Add(spec);
                db.SaveChanges();
                int latestSpecId = spec.SpecialistId;
                Note emp = new Note();
                emp.NoteId = model.notesmodel.NoteId;
                emp.NotesDescription = model.notesmodel.NotesDescription;
                emp.TakingNewPatients = model.notesmodel.TakingNewPatients;
                emp.price = model.notesmodel.price;
                emp.Waiting_Time = getwaitingtime.Waiting_Time;
                emp.SpecialistId = latestSpecId;
                emp.IsBooking = model.notesmodel.IsBooking;
                db.Notes.Add(emp);
                db.SaveChanges();
                spec.NoteId = emp.NoteId;
                db.Specialists.Attach(spec);
                db.Entry(spec).Property(x => x.NoteId).IsModified = true;
                db.SaveChanges();


                return RedirectToAction("AllSpecialist");
            }
            catch (Exception ex)
            {
                throw ex;
        ...
        public ActionResult AllSpecialist()
        {
            ...
            var data = (from st in db.Specialists
                        join sub in db.Notes on st.SpecialistId equals
                        sub.SpecialistId 

                        select new SpecialistModel
                        {
                            SpecialistId = st.SpecialistId,
                            Title = st.Title,
                            SpecialistName = st.SpecialistName,
                            ...
                            Surname = st.Surname,
                            OfficeNo = st.OfficeNo,
                            SubMenu = st.SubMenu,
                            Age = st.Age ,
                            NoteId = sub.NoteId,
                            NotesDescription = sub.NotesDescription,
                            TakingNewPatients = sub.TakingNewPatients,
                            price = sub.price,
                            Waiting_Time = sub.Waiting_Time,
                            IsBooking = sub.IsBooking,
        ...
        public ActionResult UpdateSpecialist(int id)
        {
            ...
            ViewBag.SubMenuId = new SelectList(SubMenu, "SubMenuId", "SubMenuName");
            var row = (from st in db.Specialists
                        join sub in db.Notes on st.NoteId equals
                        sub.NoteId

                       select new SpecialistModel
                        {
                            SpecialistId = st.SpecialistId,
                            SpecialistTypeId = st.SpecialistTypeId,
